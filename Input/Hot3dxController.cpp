...
Hot3dxController::Hot3dxController(_In_ CoreWindow^ window, _In_ CoreDispatcher^ dispatcher) :
    m_dispatcher(dispatcher),
    m_activeGamepad(nullptr),
    m_autoFire(true),
    m_pitch(0.0f),
    m_yaw(0.0f),
    m_state(Hot3dxControllerState::None),
...
Hot3dxController::Hot3dxController(_In_ CoreWindow^ window) :
...
void Hot3dxController::InitWindow(_In_ CoreWindow^ window)
{
    ResetState();

    window->PointerPressed +=
        ref new TypedEventHandler<CoreWindow^, PointerEventArgs^>(this, &Hot3dxController::OnPointerPressed);

    window->PointerMoved +=
        ref new TypedEventHandler<CoreWindow^, PointerEventArgs^>(this, &Hot3dxController::OnPointerMoved);

    window->PointerReleased +=
        ref new TypedEventHandler<CoreWindow^, PointerEventArgs^>(this, &Hot3dxController::OnPointerReleased);

    window->PointerExited +=
        ref new TypedEventHandler<CoreWindow^, PointerEventArgs^>(this, &Hot3dxController::OnPointerExited);

    window->KeyDown +=
        ref new TypedEventHandler<CoreWindow^, KeyEventArgs^>(this, &Hot3dxController::OnKeyDown);

    window->KeyUp +=
        ref new TypedEventHandler<CoreWindow^, KeyEventArgs^>(this, &Hot3dxController::OnKeyUp);

    // There is a separate handler for mouse only relative mouse movement events.
    MouseDevice::GetForCurrentView()->MouseMoved +=
        ref new TypedEventHandler<MouseDevice^, MouseEventArgs^>(this, &Hot3dxController::OnMouseMoved);

    SystemNavigationManager::GetForCurrentView()->BackRequested +=
        ref new EventHandler<BackRequestedEventArgs^>(this, &Hot3dxController::OnBackRequested);

    // Detect gamepad connection and disconnection events.
    Gamepad::GamepadAdded +=
        ref new EventHandler<Gamepad^>(this, &Hot3dxController::OnGamepadAdded);

    Gamepad::GamepadRemoved +=
        ref new EventHandler<Gamepad^>(this, &Hot3dxController::OnGamepadRemoved);
}

//----------------------------------------------------------------------

bool Hot3dxController::IsPauseRequested()
{
    switch (m_state)
    {
    case Hot3dxControllerState::Active:
        UpdatePollingDevices();
        if (m_pausePressed)
        {
#ifdef HOT3DXCONTROLLER_TRACE
...
bool Hot3dxController::IsFiring()
{
    if (m_state == Hot3dxControllerState::Active)
    {
        if (m_autoFire)
        {
...
bool Hot3dxController::IsPressComplete()
{
    switch (m_state)
    {
    case Hot3dxControllerState::WaitForInput:
        UpdatePollingDevices();
        if (m_buttonPressed)
        {
#ifdef HOT3DXCONTROLLER_TRACE
...
void Hot3dxController::OnPointerPressed(
...
void Hot3dxController::OnPointerMoved(
    _In_ CoreWindow^ /* sender */,
    _In_ PointerEventArgs^ args
    )
{
    ...
    case Hot3dxControllerState::Active:
        ...
        else if (pointerID == m_lookPointerID)
        {
            ...
            rotationDelta.x = pointerDelta.x * Hot3dxControllerConstants::RotationGain;       // Scale for control sensitivity.
            rotationDelta.y = pointerDelta.y * Hot3dxControllerConstants::RotationGain;
...
void Hot3dxController::OnMouseMoved(
    ...
    )
{
    ...
    case Hot3dxControllerState::Active:
        ...
        XMFLOAT2 rotationDelta;
        rotationDelta.x = mouseDelta.x * Hot3dxControllerConstants::RotationGain;   // Scale for control sensitivity.
        rotationDelta.y = mouseDelta.y * Hot3dxControllerConstants::RotationGain;
...
void Hot3dxController::OnPointerReleased(
    _In_ CoreWindow^ /* sender */,
    _In_ PointerEventArgs^ args
    )
{
...
void Hot3dxController::OnPointerExited(
    _In_ CoreWindow^ /* sender */,
    _In_ PointerEventArgs^ args
    )
{
...
void Hot3dxController::OnKeyDown(
    _In_ CoreWindow^ /* sender */,
...
void Hot3dxController::ResetState()
{
    // Reset the state of the Hot3dxController.
...
void Hot3dxController::UpdatePollingDevices()
{
    if (m_gamepadsChanged)
    {
        ...
        {
            // Hot3dxController is intended to handle input for a single player, so it
...
void Hot3dxController::ShowCursor()
{
...
void Hot3dxController::HideCursor()
{
    if (m_dispatcher != nullptr)
    {
        // Use the CoreDispatcher to ensure that the pointer is updated on the UI thread.
...
void Hot3dxController::SetMoveRect(
...
void Hot3dxController::SetFireRect(
    _In_ XMFLOAT2 upperLeft,
    _In_ XMFLOAT2 lowerRight
    )
{
...
void Hot3dxController::WaitForPress(
    _In_ XMFLOAT2 upperLeft,
    _In_ XMFLOAT2 lowerRight
    )
{
    ...
    ResetState();
    m_state = Hot3dxControllerState::WaitForInput;
...
XMFLOAT3 Hot3dxController::Velocity()
{
    return m_velocity;
}
...
XMFLOAT3 Hot3dxController::LookDirection()
{
...
void Hot3dxController::Pitch(_In_ float pitch)
{
    m_pitch = pitch;
}
...
void Hot3dxController::Yaw(_In_ float yaw)
{
    m_yaw = yaw;
}
...
void Hot3dxController::Active(_In_ bool active)
{
    ResetState();
#ifdef HOT3DXCONTROLLER_TRACE
    ...
    if (active)
    {
        m_state = Hot3dxControllerState::Active;
        HideCursor();
...
void Hot3dxController::AutoFire(_In_ bool autoFire)
{
    m_autoFire = autoFire;
}
...
bool Hot3dxController::AutoFire()
{
    return m_autoFire;
}
...
void Hot3dxController::Update()
{
    ...
    m_velocity.x = -wCommand.x * Hot3dxControllerConstants::MovementGain;
    m_velocity.z = wCommand.y * Hot3dxControllerConstants::MovementGain;
    m_velocity.y = wCommand.z * Hot3dxControllerConstants::MovementGain;
...
void Hot3dxController::OnBackRequested(
...
void Hot3dxController::OnGamepadAdded(_In_ Object^ sender, _In_ Gamepad^ gamepad)
{
...
void Hot3dxController::OnGamepadRemoved(_In_ Object^ sender, _In_ Gamepad^ gamepad)
{
    // OnGamepadAdded and OnGamepadRemoved can be called from a worker thread. For simplicity,
    // defer updating the active gamepad until Update().
    m_gamepadsChanged = true;
...
#ifdef HOT3DXCONTROLLER_TRACE
void Hot3dxController::DebugTrace(const wchar_t* format, ...)
{
